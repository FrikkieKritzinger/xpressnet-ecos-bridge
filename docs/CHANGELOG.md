...
**2026-09-02 — Phase 7 item 4 (stolen icon F0-F12) implemented and confirmed live; a deeper function-sync issue found, documented, and deliberately deferred; tagged v1.4.0**

Picked up Phase 7 item 4 (extend `PushExternalLocoUpdate()`'s "stolen icon" refresh from F0-F4 to also cover F5-F8/F9-F12) - agreed as small, low-risk, since the mechanism was already proven for F0-F4 and this just extends it using `buildFunctionGroupByte()` groups already used elsewhere. Extended the vendored `XpressNetMasterClass::PushExternalLocoUpdate()` (added `F5to8`/`F9to12` parameters, two more call-byte-then-reply message injections mirroring the F0-F4 one, same slot loop) and updated the one real call site, `XpressNetInterface::pushLocoStateToOwningSlot()`. Native suite unaffected (this file and the vendored library are both excluded from `env:native`, hardware-coupled), `env:wemos` builds clean.

**Confirmed live**: "stolen" correctly triggers on a real MultiMaus for functions tested up to F9, 100% of the time - the extension itself works.

**Investigating further (confirming the actual displayed function *value*, not just the "stolen" flash) surfaced a real, separate, and much deeper issue**: loco function state does not reliably sync between XpressNet and Z21 in either direction. This took a long live-debugging session with several dead ends before landing on the current understanding:

- **Red herring 1 - an apparent F1 toggle doing nothing.** The user reported toggling F1 on WLANmaus with no visible change on MultiMaus. A raw wire capture showed the actual byte sent was `DB3=0x89` → `TT=2 (toggle), N=9` - **WLANmaus was genuinely sending "toggle F9", not F1**, every single time, despite the user believing they pressed F1. The bridge's decode was faithfully correct; the mismatch was between the physical button and the wire command it actually produces (a WLANmaus-side function label/remap quirk, or a mis-press - not distinguished, not pursued further). This fully explained that specific symptom without any bridge-side bug.

- **Red herring 2 - erratic sync blamed on a stale post-reset state.** A capture taken immediately after a fresh board reset showed `Z21 TX: client 0 not subscribed to addr 5452 - skipping` for every attempted update - Z21's broadcast gate (`isClientSubscribed()`, populated only by `GET_LOCO_INFO`, capped at `MAX_Z21_SUBSCRIBED_PER_CLIENT`=8 with FIFO eviction, wiped on any bridge restart) was correctly explaining that specific no-op. But re-selecting the loco (forcing a fresh subscribe) and even switching to a different loco address entirely did **not** fix the underlying sync problem - ruling this out as the (or at least the only) real cause.

- **The real, still-unresolved finding**: a clean, uncorrupted capture (after several corrupted retries - the serial link was unusually flaky this whole session, more so than any prior one, needing 3-5 retries per capture at multiple points, including one reset-based capture required to clear what looked like a genuine but unconfirmed device-side stall) showed the bridge's own routing is **completely correct, every single time**: `Z21 RX: Function ... F1=1` → `Z21: Loco 20 Functions 0x00000002` → `Ecos TX: Functions ...` → `XpressNet TX: Speed ...` → `XpressNet TX: Functions - Addr=20 Fn=0x00000002` → `XpressNet TX: Pushed external update to owning slot (if any) - Addr=20` - the right bitmap, computed correctly, forwarded via both the plain broadcast (`sendFunctionCommand()`) and the push (`pushLocoStateToOwningSlot()`), unconditionally, every time. The user independently confirmed both throttles were genuinely displaying the same loco address during this test (ruling out "wrong loco selected"). With the bridge's own behavior fully verified correct at every capture, the gap is downstream - in how MultiMaus/WLANmaus actually apply a correctly-delivered update to their own display - not yet root-caused. Not investigated with a raw MultiMaus-side wire capture the way the turnout v2 saga was, for lack of time this session.

- **Confirmed unaffected**: speed/direction sync (both directions), the "stolen" flash mechanism itself (item 4's actual scope), and delivery to Ecos/DCC (confirmed 100% reliable both directions by the user directly - "the change WILL get delivered to DCC"). Only the throttle-side function *display* is affected.

- **A third red herring, investigated and fully closed**: the user separately reported the onboard activity LED's steady ~1Hz pulse (tied to `markBusActivity()`, flashing on any genuine parsed XpressNet message) had stopped, fearing a regression from today's changes. Traced to: the steady pulse the user remembered was the new `onTurnoutInfoRequest()` handler's real ~2Hz Accessory Info Request traffic from Phase 7 step 2 (2026-09-01) - which only fires while a MultiMaus is actually sitting on the turnout screen - not a general-purpose heartbeat. Since the user had moved on to testing locos (a different screen), that specific poll traffic naturally stopped, and the LED correctly went quiet except for genuine speed/direction/function commands. Confirmed live: switching back to the turnout screen brings the steady pulse back immediately. Not a bug - the user's own memory of "when was this added" was the only thing off (attributed it to a general loco-refresh mechanism that doesn't actually exist yet, rather than the turnout-specific one built the day before).

**Decision**: per the user's explicit framing ("not a critical flaw, just an annoyance" - since delivery to DCC is unaffected), the function display-sync gap is logged as Phase 7's new item 7 (deferred, low-priority) rather than chased further this session. Full write-up in `CLAUDE.md`'s Phase 7 section for whoever picks it up next - the recommended next step is the same raw-wire-capture-correlated-with-on-screen-behavior methodology that resolved the turnout v2 poll/reply mystery, which this investigation didn't get to attempt on the MultiMaus/WLANmaus receiving side specifically.

Native suite 287/287 passing throughout. `env:wemos` builds clean and is flashed and live-confirmed on the real hardware at `1.4.0` (bumped from `1.3.0` - a genuine, confirmed-live capability addition, matching this project's now-standard practice).

---

**2026-09-01 (continued) — Phase 7 step 2 (Accessory/Turnout v2, Ecos→throttle) implemented and confirmed live, tagged v1.3.0**

Same-day continuation, immediately after Phase 7 step 1's fix session. Investigation-first, per the user's request: before writing any code, confirmed against real Ecos hardware whether the individual-accessory-object API - which the official spec (`docs/ecos_pc_interface3.pdf`, section 7.12) explicitly marks `(in Planung)`/not-yet-implemented - actually works, since the user noted the spec is older than the Ecos's current firmware. Added a temporary diagnostic (`queryObjects(11, addr)` then `request(<id>, view)` for one real accessory, sent right after connect) and captured live: both worked. `queryObjects(11, addr)` returned real object IDs (DCC address 1 → Ecos ID `20000`, address 2 → `20001`), and subscribing to one individually delivered real `state[val]`/`switching[val]` events on every real throw on Ecos. This directly validated the user's hunch and unblocked the whole feature - confirmed real, not planned-but-unshipped, contrary to the spec text.

**Implementation, mirroring the loco address-map/subscribe pattern**: `EcosInterface` gained a second, separate address map (`accessory_address_map[]`) - separate because `queryObjects(10, addr)` (locos) and `queryObjects(11, addr)` (accessories) produce identically-shaped replies (object ID + addr, no speed/dir/func) with no way to tell which command a given reply answers from the protocol itself. Disambiguated via a new `ECOS_ACCESSORY_ID_MIN` threshold (10000) - real, structural, and confirmed via the live capture (locos sat in the 1000s, accessories at 20000+, different manager object pools by design, not coincidental to this one layout). Queried once per connection (not on a refresh timer like locos - accessories are added far less often) and subscribed one accessory per `update()` tick (same non-blocking pacing discipline as the loco baseline query, Phase 6 step 4's hard-won lesson). `EcosMessageParser` gained `state`/`switching` property parsing - `switching` deliberately ignored (a transient in-motion flag bracketing the transition, confirmed via capture) - and a new `CommandRouter::handleEcosAccessoryCommand()` forwards a real state change to both XpressNet and Z21 unconditionally (no originating protocol to skip, unlike the v1 throttle→Ecos direction) - and deliberately does NOT touch `last_accessory` (the OLED line), per the user's explicit design call earlier the same day that it should reflect handheld-commanded state, not Ecos's truth.

**Three real bugs, found in order, each requiring live testing to catch:**

1. **`state[val]`'s polarity was backwards.** Initial implementation guessed `state=1` = diverging. The user reported no visible change on either throttle after flashing; a live capture correlating the bridge's own `set(11, switch[...])` commands against the resulting `state[val]` events showed a consistent, 100%-repeatable inversion across 8 cycles (command straight → event reports diverging, every time). Flipped (`state=0` = diverging).

2. **A plain `SetTrntPos()` broadcast to XpressNet did nothing on real hardware**, even with the polarity fixed. Verified via live capture that the bridge's own code was executing correctly (`XpressNet TX: Accessory...` logged every time) - so the gap was the real MultiMaus not trusting an unsolicited broadcast, not a bridge encoding bug. Added `XpressNetMasterClass::PushExternalTurnoutUpdate()` to the vendored library, mirroring `PushExternalLocoUpdate()`'s call-byte-then-unmarked-reply bus injection (Phase 5 step 9) - unlike the loco version, doesn't touch `AddBusySlot`/`SlotLokUse` at all (no ownership/stealing concept for turnouts). Still didn't fix it (see bug 3) but is a real, reasonable addition, kept.

3. **The actual root cause: real XpressNet devices use poll/reply for turnout state, not push/broadcast - and this bridge never implemented the poll side at all.** After the push fix still didn't work, and after ruling out "screen must be open" via a controlled test (both throttles on the turnout screen, T0001 selected, toggling from each side while watching both), the user proposed testing against a **real Roco Z21 command station with two MultiMaus on X-Bus** - genuinely decisive ground truth. Results: changing a turnout on either throttle updated the *other* throttle's display too, but with a **1-2 second delay** (not instant), and reselecting a turnout correctly restored its last-known state. Both are hallmarks of a poll-driven mechanism, not bus-snooping. This pointed directly at `notifyXNetTrntInfo` - the Lenz XpressNet "Accessory Decoder Information Request" (header `0x42`/`0x43`, distinct from the `0x52` SET command `notifyXNetTrnt` already handles) - which had never been hooked up to anything at all. Added a temporary diagnostic hookup and confirmed live: a real MultiMaus polls this **~2Hz** while sitting on a turnout screen (115 hits in a 60s capture), previously getting zero reply, ever - directly explaining "falls back to unknown after a few seconds" for every single symptom description across this whole session.
   - Implemented `XpressNetInterface::onTurnoutInfoRequest()` → `XpressNetMasterClass::SetTrntStatus()`. The reply format is non-trivial: turnouts are addressed in *groups* of 4 (2 per nibble), with an `I`(incomplete)/`TT`(feedback type)/`N`(nibble select)/`Z3Z2Z1Z0`(two 2-bit position fields) byte layout. Rather than guess, dispatched a research agent to cross-reference the official Lenz XpressNet Specification (v2, 6/2003, sections 2.1.11/2.2.17) against the vendored library's own source comments and a second independent XpressNetMaster fork (Digital-MoBa) - all three agreed, and the live capture's own observed request (`Address=0, data=0x80` for turnout #1) matched the derived group/nibble math exactly (group 0 = turnouts 0-3, nibble 0 = the first two). The straight-vs-diverging → `01`-vs-`10` relative-label mapping isn't defined by the spec (it says so explicitly) - picked to match this project's existing Z21 `position_byte` convention (01=straight, 10=diverging) for consistency.
   - **Confirmed live**: MultiMaus no longer falls back to "unknown" - holds correct state, and correctly reflects a change made from Z21 too (served on its next poll from the same state cache).
   - Added `CommandRouter`'s new best-known-accessory-state cache (`known_accessories[]`/`getKnownAccessoryState()`, distinct from `last_accessory` as above), updated by both the v1 and v2 directions - needed to answer this XpressNet request, and reused to fix Z21's `GET_TURNOUT_INFO` reply too (previously hardcoded to always report "straight").

**A real, deferred-but-not-fully-understood limitation found along the way**: while diagnosing why a Z21-initiated change wasn't reaching WLANmaus's own display even after all of the above, a live capture found a real WLANmaus sends **zero** `GET_TURNOUT_INFO` requests in 60 seconds sitting idle on the turnout screen - contradicting an earlier session's own comment claiming it polls at 2+Hz (that assumption, now confirmed wrong, apparently drove the original err13 rate-limiting fix in Phase 7 step 1 for a different reason than actually applied). With no polling fallback, WLANmaus relies entirely on receiving the `LAN_X_TURNOUT_INFO` broadcast - already independently confirmed byte-perfect and delivered to the correct connected client every time, yet its display almost never refreshed from it.

One exception, found right at the end of this session: after leaving the throttles idle for a while, an Ecos-direct change *did* eventually reach WLANmaus's display, a few seconds later - not reproduced again despite repeated attempts. That single success rules out "WLANmaus categorically ignores this broadcast" as the explanation, but a generic UDP-packet-loss theory doesn't cleanly fit either, since loco broadcasts use the identical one-shot, no-retry UDP send and have been reliable throughout this entire project's history - something turnout-specific is still going on, cause unconfirmed. XpressNet already works reliably in both directions, which the user is satisfied with for now - deferred as Phase 7's new item 6 (open/TBD-style, not blocking) rather than chased further blind this session.

**Methodology note**: this session is a clean illustration of the value of real reference hardware over continued guessing. Two plausible-looking fixes (state polarity, then the push mechanism) each required live testing to confirm or rule out, and neither actually solved the reported symptom - it took bringing in a genuinely independent, known-good reference (a real command station) to reveal that the whole mental model (push/broadcast) was wrong, not just some byte in it. The org-established "prefer a wire capture over reasoning from logs" lesson held again here, one level up: prefer a working reference implementation over reasoning about wire captures when even the captures are consistent with multiple different underlying mechanisms.

Native suite grew from 272 to 287 (regression tests for the accessory decode, state cache, and Ecos-sourced router forwarding); `env:wemos` builds clean (no material change in RAM/Flash headroom) and is flashed and live-confirmed on the real hardware. Tagged `v1.3.0`, bumped from `1.2.0` - the user's now-standard practice of bumping for any confirmed-live capability addition.

---

**2026-09-01 — Phase 7 step 1 (Z21 turnout/accessory support) fully fixed and confirmed live, tagged v1.2.0**

Picks up from a separate session's checkpoint (`b7aad81`, explicitly marked "partial") where the user reported: WLANmaus turnout control was live-tested with two turnouts (T0001, T0002) and produced clearly wrong behavior - pressing "straight" on T0002 worked, but releasing the button reverted it to diverging; pressing "diverge" on T0002 instead affected T0001; and switching the selected turnout on the WLANmaus had no effect on which address actually got commanded ("as if the address has not changed"). The user was frustrated that a prior debugging attempt (in that other session) had concluded this was a WLANmaus UI bug rather than a bridge bug, and asked this session specifically to cross-check the implementation against Philipp Gahtow's own reference Z21 project (the same author as this project's vendored XpressNetMaster library) as a second source of truth.

**Bug 1 - `LAN_X_SET_TURNOUT`'s DB field order was backwards, and a prior fix attempt entrenched it instead of correcting it.** The original implementation (`0861f29`/`2f707f7`, 2026-08-31) assumed the flags byte (`10Q0A00P`) came first and the address bytes second, copying the field order already proven correct for `LAN_X_SET_LOCO_DRIVE`/`SET_LOCO_FUNCTION`. The real spec (section 5.2) puts address bytes first and the flags byte third for this specific command - confirmed two ways: the spec's own table text, and a verbatim `curl` download (not an AI-summarized fetch, to rule out a paraphrasing error) of `kind3r/esp8266-z21-lib`'s `z21.cpp`, a direct port of Gahtow's original library: `notifyz21Accessory((packet[5]<<8)+packet[6], bitRead(packet[7],0), bitRead(packet[7],3))` - address bytes (`packet[5]`/`[6]`) before the flags byte (`packet[7]`). With the wrong order, the decoded "address" was actually built from the flags byte - a function of which physical button was pressed, not which turnout was selected on the throttle - matching the reported symptom exactly (address effectively fixed per-button, and "as if the address hasn't changed" when switching turnouts). A separate debugging session (between `2f707f7` and `b7aad81`) noticed the resulting garbage and, instead of finding the byte-order bug, rewrote `z21DecodeTurnoutCommand()` to match the (mis-assigned) bytes it was actually receiving and added a "release-pulse debounce" heuristic (matching same-address-different-diverging-bit patterns within 100ms and discarding them) to paper over the resulting flicker - the debounce hack correctly suppressed some symptoms without addressing the actual cause, which is what led the other session to conclude the WLANmaus itself was misbehaving. Restored the correct spec-based decode (address bytes first, flags byte third, ignore the deactivate/release edge entirely rather than debouncing it - exactly matching XpressNet's own already-proven `onTurnoutCommand()` pattern from Phase 5 step 10) and removed the debounce mechanism and its now-unused `Z21Client::last_turnout_lsb`/`last_turnout_lsb_time_ms` fields entirely.

*(Separately, the other session had also correctly found and fixed a real, different bug in the same area while chasing what turned out to be err13 from a hot-path `DEBUG_PRINTF`: `LAN_X_GET_TURNOUT_INFO`/`LAN_X_TURNOUT_INFO` were using the wrong X-Header `0xE3`/`0xEF` (copied from `GET_LOCO_INFO`/`LOCO_INFO`) instead of the spec's actual `0x43` for both, plus a `DEBUG_PRINTF` on that now-correctly-matching, frequently-polled query path that caused genuine XpressNet err13 under real load - fixed with rate-limiting (1/sec per client) and removing the debug print, mirroring this project's own established `LAN_X_GET_STATUS` precedent. Both of those fixes were correct and left untouched by this session.)*

Flashed the fix and asked the user to retest - they reported it was still completely silent: WLANmaus registered the button press on its own screen, but nothing reached Ecos for either address, in either direction.

**Bug 2 - the "is this a valid turnout flags byte" nibble-format check didn't correctly mask out the Q (queue) bit.** Re-deriving the check bit-by-bit: `10Q0A00P`'s upper nibble is `1000` (Q=0) or `1010` (Q=1) - a correct "ignore Q, require the rest" mask needs to clear nibble bit1 (`0x0D`), but the carried-forward mask was `0x0E` (clears nibble bit0 instead, which is a fixed-always-0 bit anyway, so it does nothing useful) - meaning any packet with `Q=1` (queued mode, a real feature from Z21 firmware ≥1.24) was silently rejected outright, regardless of address or button. Fixed the mask, added a regression test for the queued case. Flashed again - user reported still no effect.

**Bug 3 - this specific real WLANmaus doesn't set the documented `bit7` prefix at all, confirmed via a raw wire-byte capture.** With two plausible-looking bugs fixed and still no live effect, went straight to this project's own proven diagnostic method rather than guessing further: added a temporary raw hex dump of every incoming `LAN_X` dataset (network-side UDP dispatch only, not the XpressNet UART hot path that caused the Phase 5 step 10 incident, so safe to run continuously), flashed `env:debug`, and captured 45 seconds of live serial output via a background `pyserial` script (`dtr=False`/`rts=False`, matching the established pattern) while the user pressed T0001 and T0002's buttons. Real captured `SET_TURNOUT` packets: `53 00 00 09 80`, `53 00 00 01 80`, `53 00 00 08 80`, `53 00 00 00 80`, `53 00 01 09 80`, etc. - `data[1]`(Adr_MSB) confirmed always `0x00`, `data[2]`(Adr_LSB) confirmed correctly `0x00`/`0x01` for T1/T2 (validating bug 1's address-order fix), but `data[3]`(flags) was `0x00`/`0x01`/`0x08`/`0x09` - never the spec-documented `0x80`/`0x81`/`0x88`/`0x89` with `bit7` set. Verified this isn't noise: reading `bit3`(activate)/`bit0`(port) directly from these values produces a fully coherent press→release, straight/diverging sequence matching exactly what the user did (e.g. `09`→`01` = diverging Activate→Deactivate; `08`→`00` = straight Activate→Deactivate). Removed the nibble/format gate entirely - `X-Header 0x53` alone already disambiguates this as a turnout command (unlike `SET_LOCO_FUNCTION`, which genuinely shares its X-Header with `SET_LOCO_DRIVE` and needs one) - and read `bit3`/`bit0` unconditionally, trusting the real capture over the literal spec text. Same category of decision this project has made before (the Z21 direction-bit inversion, the XpressNet accessory r/g port-letter mixup): real hardware wins over a spec-literal reading when they conflict. Removed the temporary hex-dump diagnostic once confirmed.

**Confirmed live**: T0001 and T0002, both buttons, both directions, no cross-addressing, no reverting on release, no err13. 4 new/rewritten native tests (turnout decode tests updated to the correct byte order and real flags-byte values; added a deactivate-edge-ignored test and an "ignores upper bits" robustness test; removed the now-inapplicable "rejects non-turnout flags byte" test). Native suite 272/272 passing throughout (dipped to 273 mid-session with the Q-bit fix's regression test, settled at 272 once the bit7-gate removal made that test's premise moot and it was folded into the "ignores upper bits" test instead). `env:wemos` flashed at `1.2.0` (already bumped by the other session for this feature - left unchanged, since this is the same shipped capability being fixed, not a new one).

**A note on process**: this whole saga is a clean example of two different debugging postures reaching opposite conclusions from the same symptom. Pattern-matching a wrong byte-order onto an already-proven-correct sibling command (bug 1) produced plausible-looking wrong output; treating that wrong output as ground truth and rewriting the decoder to match it (rather than questioning the call site) locked the bug in and shifted blame onto the WLANmaus itself. Cross-checking against a second independent source (the official spec text plus a real reference implementation, not just re-reading this project's own code more carefully) is what actually broke the deadlock - and even then, two further real bugs (the Q-bit mask, and the genuinely nonstandard real-hardware flags byte) still needed an actual raw wire capture to find, not further spec-reading. Reasoning about protocol bytes from memory/spec text alone is fast but not sufficient once reality diverges from documentation - this project's own established "prefer a wire capture over reasoning from log lines" lesson (Phase 6 step 4) held again here, one level down at the raw-byte level.
...
**Real bug found immediately in live testing**: "Last Msg" stayed stuck near 0 even after the throttle sat completely idle for 20+ seconds. Added targeted debug logging (`now`/`last_message_time`/`age` on every call) and captured it live: the timestamp was genuinely updating every ~500ms-1s, continuously, throttle untouched. Traced to real WLANmaus hardware polling `LAN_X_GET_STATUS` (a real, spec-documented request, likely checking track power) roughly twice a second even at rest - handled correctly by the existing dispatch code, just without a debug print on that branch, which is why it stayed invisible in every earlier wire capture during this project's testing (filtered for the wrong markers). Not a bug in the age arithmetic - the field was doing exactly what "time since last packet" means - but not a useful "did someone touch the throttle" signal either, which is what the user actually wanted once the real cause was understood. Fixed by moving the tracked timestamp from `handlePacket()` (fires for every packet, keep-alive polls included) to individual `recordAction()` calls at the drive/function/emergency-stop/track-power branches only - each client's own `last_seen_ms` (used for the unrelated 60s connection-timeout expiry) is untouched, since THAT legitimately should count any packet, status polls included, as "still alive."

`ComponentStatus`/`getActiveClientCount()`/`getLastMessageAgeMs()`/`getLastMessageSourceIp()` added as new virtuals on `ProtocolInterface` (default no-ops), following the same pattern already established for `pushLocoStateToOwningSlot()` - lets `CommandRouter` stay uniform over a base-class pointer while only `Z21LanInterface` actually implements them. 4 new native tests covering the new `SystemStatus` fields (`z21_status`/`z21_client_count`/`z21_last_message_age_ms`/`z21_last_message_ip`), using new `MockProtocolInterface` setters; native suite 258/258 passing. `env:wemos` flashed to the real hardware at `1.1.1` (patch bump from 1.1.0 - the user's choice, since this is a fix/addition to existing functionality rather than a new capability). Tagged `v1.1.1` and pushed.

---

**2026-08-28 (continued) — Phase 6 step 4 CLOSED OUT: direct cross-protocol fan-out redesign, nine more real bugs, tagged v1.1.0**

Direct continuation of the same day's marathon session below, after a checkpoint pause. WLANmaus and a tethered MultiMaus stayed connected simultaneously throughout. Same methodology as the first half: live-test, capture serial output, trace root cause in code before changing anything, fix, rebuild, reflash, re-test - now also using raw wire-byte hex dumps (both outgoing Z21 packets and incoming Ecos property lines) for several of the harder bugs, since log-line reading alone wasn't resolving them.

- **The architectural turning point**: testing resumed with the step-4-checkpoint architecture still in place - a throttle-facing protocol's command reached the *other* throttle-facing protocol only indirectly, by waiting for Ecos's own echo to complete the loop (`broadcastCommand()`'s `ECOS` branch fanning out to both). Live testing kept finding that this loop just didn't reliably close: XpressNet→Ecos worked, Z21→Ecos worked, but XpressNet↔Z21 (via each other's Ecos echo) essentially never worked, no matter what was fixed in the echo-timing logic. The user asked directly: *"why do we rely on ECOS response to broadcast to other protocols? ... if xnet changes speed, we broadcast to ECOS and Z21 ... it seems that we are now building the rebroadcast logic on what we get back from ECOS?"* - and proposed the fix: broadcast to all other protocols immediately from whichever one originated the change, excluding only the source. Confirmed compatible with the existing main-loop structure and priority ordering (`xpressnet_ecos_bridge.ino`'s `loop()`: XNet polled first every iteration since it's most timing-sensitive, then Ecos/LocoNet/Z21, then low-priority housekeeping/OLED - already matches the intended design, OLED deliberately staying decoupled on its own timer rather than updating inline per-message to avoid reintroducing a blocking-burst risk). Implemented in `CommandRouter::broadcastCommand()`: the `XPRESSNET`/`Z21_LAN` branch now sends to Ecos *and* directly to the other throttle-facing protocol in the same call, unconditionally. The `ECOS` branch is now needed only for genuine Ecos-originated changes (someone touching Ecos directly) - it still exists, unchanged in shape, since that's the only path a change made directly on Ecos can ever reach the bridge through.

- **Bug 9 - `EcosInterface::sendFunctionCommand()` had the exact same blocking-burst anti-pattern as the baseline query (bug 7 from the first half), just never paced.** Diagnosed from a live capture showing Ecos replying `control[none]` (twice) instead of any real state confirmation, right after a single XpressNet function toggle - all 32 individual `set(func[n,v])` commands were being written to the TCP socket synchronously in one call. Initially fixed by pacing it the same way as the baseline query (one `set()` per `update()` tick, via `pending_function_object_id`/`pending_function_bitmap`/`pending_function_step`) - this masked the symptom but introduced a new one (see bug 12).

- **Bug 10 - the router's own echo-prevention was ALSO independently swallowing the baseline `get()` replies from bug 6/7, on top of `EcosInterface`'s already-fixed local gate.** `CommandRouter`'s `isEchoCommand()` treated any Ecos reply within 500ms of *any* recent command for that address as an echo of whichever protocol sent it - and since a paced 32-reply baseline sweep takes real wall-clock time (confirmed live: exactly the first ~21 of 32 replies were suppressed, matching how long 21 round trips takes to clear a 500ms window), most of a sweep kept landing inside a stale window. Initially patched with a `bypass_echo_check` parameter keyed off `EcosReply::Kind::REPLY` (assuming a direct response to our own `get()`/`set()`/`request()` is never a real echo) - live evidence showed Ecos's own REPLY/EVENT framing for `get()` didn't reliably match that assumption, so this was replaced with a fully deterministic check instead: `baseline_query_object_id`, which the bridge itself controls precisely, with no dependency on guessing Ecos's wire framing.

- **Bug 11 - the deeper form of bug 10, and the reason cross-protocol propagation stayed broken even with bugs 9-10 fixed**: `handleEcosCommand()`/`handleEcosFunctionCommand()`'s echo check was all-or-nothing - correct for not echoing (say) XpressNet's own command back to XpressNet, but with Z21 now also in play, dropping the ENTIRE update also threw away the copy meant for Z21 (or vice versa). This is what the direct-fan-out architectural change (above) was designed to route around, but the underlying gate was also just wrong on its own terms. Fixed properly: removed the early-return echo gate from both handlers entirely, and added `CommandRouter::wasRecentSource(address, source)`, checked per-destination inside `broadcastCommand()`'s `ECOS` branch - skip forwarding only to whichever protocol was the actual recent originator, always forward to the other one. Real architectural fix, not a patch - generalizes correctly to however many throttle-facing protocols this bridge ever supports (LocoNet included, when built). The now-fully-redundant, separately-buggy `EcosInterface`-level echo mechanism (`echo_queue`/`addToEchoQueue()`/`isEchoCommand()`, `ECHO_TYPE_SPEED`) was removed as dead code in the same pass.

- **Bug 12 - bug 9's pacing fix introduced a NEW intermittent-failure mode**: rapid successive real button presses (exactly what happens toggling F8 through F11 in quick succession, which is how the user was testing) kept restarting the 32-step paced sweep before it could finish - most presses in a burst never got far enough to complete at all before being superseded by the next one, so only roughly the last one ever actually reached Ecos ("about 1 in 4 toggles seem to update", matching the log evidence precisely: a burst of five group-2/group-3 XpressNet function messages, only the last of which fully landed). Root-fixed properly instead of re-tuning the pacing: added `last_sent_functions`/`has_last_sent_functions` to `EcosInterface`'s per-loco `AddressMapEntry`, and rewrote `sendFunctionCommand()` to diff the new bitmap against what was actually last sent - only the bit(s) that changed (typically one) get sent, immediately, synchronously, no pacing needed at all for the common case. Pacing is now used only for the genuine first-ever send to a freshly-subscribed loco, where there's nothing yet to diff against.

- **Bug 13 - the exact same all-or-nothing echo pattern as bug 11 also existed, redundantly, on the *incoming* side of all four throttle-protocol handlers** (`handleXpressNetCommand`, `handleXpressNetFunctionCommand`, `handleZ21Command`, `handleZ21FunctionCommand`), gating genuine new incoming commands rather than outgoing forwarding. Diagnosed from the user's report that XNet updates felt "sluggish" and function toggles were intermittently just not registering, worse than before the direct-fan-out change - because that change made Ecos's own confirmations reach back to XpressNet far more reliably than before, `echo_state` was now getting touched by many more sources, increasing the odds any given genuine new command would collide with a recent unrelated touch and get silently dropped. Removed all four gates outright after concluding they were never protecting against a real physical risk: XpressNet is master-polled (the vendored library's `notify*` callbacks only fire for genuine polled throttle replies, never for the bridge's own outgoing transmissions), and Z21 is UDP client-server with no possible echo path at all. `wasRecentSource()` (bug 11) still correctly protects the outgoing direction.

- **Bug 14 - `handleEcosCommand()`/`handleEcosFunctionCommand()` broadcast unconditionally, even when the Ecos event just re-confirmed a value the bridge already had** (a redundant echo of the bridge's own background traffic - baseline queries, direct fan-out, Z21 activity - not a genuine change). Every one of these no-op re-broadcasts still called `pushLocoStateToOwningSlot()`, which a real MultiMaus visibly flashes "stolen" for even when nothing changed. Added a real "did this field actually differ from what StateEngine already had" check before broadcasting in both handlers.

- **Bug 15 - `ECHO_PREVENTION_WINDOW` (500ms) was simply too short for Ecos's real confirmation round trip under actual load** (TCP latency, the diff-based function sends, general contention) - a protocol's own command's confirmation could legitimately arrive after the window closed, get misread by `wasRecentSource()` as an independent Ecos-side change, and get pushed straight back to that same protocol as if external (XpressNet flashing "stolen" for its own command). Widened to a new `ECOS_ECHO_ATTRIBUTION_WINDOW_MS` (4000ms) - the now-fully-unused `isEchoCommand()` (superseded by `wasRecentSource()` in bug 11) was removed as dead code in the same pass, along with the now-stale `ECHO_PREVENTION_WINDOW` constant.

- **Bug 16 - the true root cause of "stolen" still recurring even after bug 15's wider window, confirmed via a fully isolated test** (the user powered WLANmaus off entirely, leaving only XpressNet and Ecos connected, and still saw XpressNet flash "stolen" for its own commands while Ecos was applying them): `handleEcosCommand()`/`handleEcosFunctionCommand()` unconditionally stamped the shared `echo_state.last_source = ECOS` at the end of every successful call - including when the update had just been correctly attributed back to a recent XpressNet/Z21 command. If XpressNet sent two commands close together (e.g. a drive command then a function command), the FIRST one's Ecos confirmation would overwrite `echo_state` to `ECOS`; when the SECOND command's own confirmation arrived shortly after, it no longer looked like an echo of a recent XpressNet command, and got pushed straight back to XpressNet as if external. Nothing in the codebase ever actually checks `echo_state.last_source == ECOS` - `wasRecentSource()` is only ever called with `XPRESSNET`/`Z21_LAN` - so the stamp served no purpose and only caused harm. Fixed by simply no longer touching `echo_state` from Ecos-sourced handling at all. Confirmed live, isolated: no more spontaneous "stolen", "much more responsive", no more missing messages - then reconfirmed with WLANmaus back on, full six-direction matrix (XpressNet↔Ecos, Z21↔Ecos, XpressNet↔Z21) all working.

- **F8-F11 appearing inverted between Ecos and the bridge, on multiple locomotives, not just one** - investigated at length after the earlier per-loco-decoder-compensation theory (a real, precedented pattern already documented for this project's `dir` handling) was ruled out by the user (reproducible across many locos, not flagged as inverted in Ecos's own UI). Re-verified both halves of the pipeline byte-exact: the Z21 encoder (`Ecos RX raw: 1009 func[8,1]` → `Z21 TX: bytes = ... 08 00 00 ...`, `0x08` = bit3 of DB5, exactly where the spec places F8) and the Ecos property parser (`EcosMessageParser`'s `func[n,v]` handling, fully generic over 0-31, no special-casing for any index) both confirmed mathematically correct with no possible source of a selective 4-bit inversion. The user then found the actual cause directly in Ecos: F8-F11 are genuinely inverted at the Ecos/decoder level for the locos in question. Not a bridge bug - closed as confirmed non-issue.

**Closing state**: all six throttle-pairing directions (XpressNet↔Ecos, Z21↔Ecos, XpressNet↔Z21) confirmed working live, each verified with the other two protocols fully powered off to rule out cross-contamination in the test itself. No XpressNet bus errors (err13) under real simultaneous multi-protocol load. Native suite 254/254 passing throughout every fix. `env:wemos` (production, not `debug`) built and reflashed to the real hardware for the first time since the marathon session began, at `FIRMWARE_VERSION 1.1.0` (bumped from `1.0.0` - the user's choice among minor/patch/major, since this added a genuinely new capability - Z21/WLANmaus support - on top of the existing feature set). Tagged `v1.1.0` and pushed.

---

**2026-08-28 — Phase 6 step 4: marathon live WLANmaus + tethered MultiMaus session, eight real bugs found and fixed, one item still open**

A single very long session picking up from the 2026-08-27 checkpoint (Z21 code-complete but never live-tested). Both a real WLANmaus and a tethered MultiMaus were connected simultaneously for cross-protocol testing. Methodology throughout: live-test a symptom, capture serial output via a background `pyserial` script (`dtr=False`/`rts=False` before `open()`, matching `platformio.ini`'s own monitor settings, to avoid triggering an extra board reset), extract readable lines even from partially-garbled captures, trace the exact root cause in code before changing anything, fix, rebuild, reflash, re-test. Several capture rounds came back corrupted (repeating byte patterns, a known flaky-serial issue on this board unrelated to firmware) and were simply retried.

- **Bug 1 - `LAN_X_GET_LOCO_INFO` request decode used the wrong byte offsets.** (This was actually found and fixed right at the start of this session, continuing directly from the prior checkpoint - documented here for completeness since it's what unblocked everything else.) `Z21LanInterface::handleDataset()`'s `Z21_X_GET_LOCO_INFO` branch read `data[1]`/`data[2]` (DB0=`0xF0` and Adr_MSB) instead of `data[2]`/`data[3]` (Adr_MSB and Adr_LSB), decoding a garbage address (`0x3000 | actual Adr_MSB`) for every subscription request. WLANmaus's own commands reached Ecos fine (that path decoded the address correctly), but WLANmaus itself never received any `LAN_X_LOCO_INFO` reply for the address it actually subscribed to - matching the reported symptom exactly (commands work, own display never updates). Fixed; confirmed live across 9+ different loco selections.

- **Bug 2 - Z21 function-command confirmation relied entirely on an Ecos round trip that gets echo-suppressed for the very client that sent it.** After bug 1's fix, WLANmaus could send function toggles and Ecos received them, but WLANmaus's own screen never updated. Traced to `broadcastCommand()`'s existing echo-prevention: when WLANmaus sends a command, `echo_state.last_source` becomes `Z21_LAN`; when Ecos's own confirmation event arrives shortly after, the router correctly recognizes it as an echo of what Z21 itself just sent and (at the time) dropped it entirely - fine for XpressNet, which optimistically updates its own display locally without waiting for confirmation, but WLANmaus does *not* do that for function buttons. Real Z21 hardware, per the official spec, confirms `SET_LOCO_DRIVE`/`SET_LOCO_FUNCTION` by broadcasting `LAN_X_LOCO_INFO` to subscribed clients *immediately*, synchronously with processing the request - it has no DCC ACK to wait for. Added `Z21LanInterface::broadcastConfirmedState()`, called right after `handleZ21Command()`/`handleZ21FunctionCommand()`, mirroring that real-hardware behavior instead of depending on the Ecos round trip.

- **Bug 3 - real WLANmaus hardware never sends `LAN_SET_BROADCASTFLAGS` at all.** Even after bug 2's fix, added debug logging (`Z21 TX: client N active but broadcast flags 0x00000000 don't include 0x1 - skipping`) showed every outgoing confirmation being silently dropped because the client's `broadcast_flags` was always `0`. Confirmed via two full capture rounds, including after a complete WLANmaus reboot, that it never sends the spec's documented opt-in flag - it apparently treats "subscribed via `LAN_X_GET_LOCO_INFO`" as sufficient on its own. Fixed by defaulting new clients' `broadcast_flags` to `0x00000001` (driving/switching broadcasts) in `findOrAddClient()`, rather than requiring the explicit opt-in the spec describes; a client that *does* explicitly call `LAN_SET_BROADCASTFLAGS` (e.g. a future PC client) still overrides the default either way.

- **Bug 4 - Z21's direction bit was decoded per the literal spec meaning, but needed inverting to match XpressNet's already-proven convention.** After bugs 1-3, WLANmaus correctly received confirmed state - but its direction arrow showed the opposite of what Ecos and the (already-validated) MultiMaus showed, for the *same loco* that XpressNet already handles correctly. Verified byte-for-byte against the actual Z21 spec PDF (`docs/z21-lan-protokoll-en.pdf`, section 4.2): "R ... Direction: 1=forward" - `z21DecodeSpeed()`/`z21EncodeSpeed128()` were implementing this literally and correctly. But XpressNet's own decode (`xpressnet_interface.cpp`) inverts its wire bit (`(speed_byte & 0x80) ? 0 : 1`), per the real Lenz XpressNet spec, and that path is what's proven correct against this real Ecos+loco setup (live-validated 2026-07-31). Both decodes are individually spec-correct for their own real protocol, but the two real specs use opposite bit conventions for "forward" - so the un-inverted-passthrough quirk already documented in `ecos_message_parser.cpp` (ESU's own spec says Ecos's `dir` should be inverted too, but passing it through unchanged is what actually matches this loco) only cancels out correctly for XpressNet's already-inverted decode. Inverted `z21DecodeSpeed()`/`z21EncodeSpeed128()` to match XpressNet's convention instead of the Z21 spec's literal text, since empirically matching what's already proven correct wins over spec technicalities here (same class of decision as the existing Ecos `dir` quirk). Confirmed live: direction now shown consistently between Ecos, MultiMaus, and WLANmaus.

- **Bug 5 - a plain speed/direction command was unconditionally re-sending the loco's entire function bitmap downstream too.** While chasing a report that F8/F9/F10/F11 were desynced from Ecos on WLANmaus (and unfixable by re-selecting the loco), found that `CommandRouter::broadcastCommand()`'s `XPRESSNET`/`Z21_LAN` branch called `ecos->sendFunctionCommand()` unconditionally on *every* drive command, not just real function changes - harmless once the bridge's internal function state matches Ecos's, but StateEngine starts every loco at `functions=0` on boot, and the very first drive command from a reconnecting throttle (which triggers the loco's Ecos subscription) fires before Ecos's own reply could populate the real value - so that first drive command blasted an all-zero bitmap at Ecos, silently wiping out whatever real function state (sound toggles, etc.) it already had. Added a `functions_changed` parameter to `broadcastCommand()` (default `true`, so the function-command handlers are unaffected), passed `false` from all three drive-only handlers (`handleXpressNetCommand`, `handleZ21Command`, `handleEcosCommand`). This alone didn't fully fix the F8-11 symptom (see bug 8), but is a real, independently-worth-fixing correctness bug on its own.

- **Bug 6 - no baseline query for a freshly-subscribed loco's real state.** Continued digging into the F8-11 desync with targeted logging (`Z21 RX: Function raw DB3=... current-bridge-state=...`) and found the bridge's own internal belief for F8 was `0` even though Ecos genuinely had it `1`. Traced through the real ESU PC-Interface spec (`docs/ecos_pc_interface3.pdf`, section 7.11): `request(id, view)`/`request(id, control)` only subscribe to *future* change events - they never report the loco's state as it stood before subscribing, and the real protocol has no bulk "give me everything" query; each function has to be asked for individually via `get(id, func[nr])`. This bridge never sent any `get()` command at all - `ecosBuildGetCmd()` had been deleted as apparently-unused dead code during the Phase 5 step 1 audit, not realizing at the time that it was actually a missing feature rather than genuinely dead. Added `ecosBuildGetPropertyCmd()`/`ecosBuildGetFunctionCmd()` and wired `EcosInterface::subscribeToLoco()` to query `speed`, `dir`, and all 32 `func[nr]` (34 queries total) right after subscribing.

- **Bug 7 - sending those 34 baseline queries as one synchronous burst blocked the main loop long enough to break real XpressNet bus timing.** The first live retest of bug 6's fix (with a tethered MultiMaus actively polling the bus, for the first time this session) showed the MultiMaus struggling to sync, staying in err13, and needing a full board reset to recover - plus function toggles that visually worked on WLANmaus's own screen but never actually reached Ecos, and no "stolen" refresh on MultiMaus at all. Root cause: `subscribeToLoco()` was writing all 34 `get()` commands to the TCP socket in one call, which - even though each individual `wifi_client.write()` is fast - collectively took long enough to starve `XpressNetInterface::update()`'s time-critical bus polling, exactly the failure mode `CLAUDE.md`'s non-blocking design rule exists to prevent. This is very likely a pre-existing latent risk in `EcosInterface::sendFunctionCommand()`'s own 32-`set()`-command loop too (same anti-pattern, used for years), just never exercised with a real, actively-polling MultiMaus simultaneously present until this exact test. Fixed by pacing the baseline query to one `get()` per `update()` tick (`EcosInterface::sendNextBaselineQueryStep()`, driven by `baseline_query_object_id`/`baseline_query_step` state) instead of a synchronous burst. Confirmed live: MultiMaus stayed responsive and synced throughout, no err13 recurrence.

- **Bug 8 - the router's echo-prevention was a single shared gate that dropped an Ecos-originated update entirely, not just for the originating protocol.** Two related manifestations, found and fixed across several rounds:
  - First surfaced as the baseline queries from bug 6/7 *still* getting swallowed even after pacing - `CommandRouter`'s own `isEchoCommand()` check (separate from `EcosInterface`'s own, already-relaxed one) treated any Ecos reply for an address within 500ms of *any* recent command for that address as an echo, regardless of which protocol actually sent it - and since subscribing fires synchronously off a drive command, and paced baseline queries take real wall-clock time to complete (confirmed via capture: exactly the first ~21 of 32 replies were suppressed, matching how long 21 round trips takes to clear a 500ms window), most of the baseline sweep kept landing inside that window. Initially patched with a `bypass_echo_check` parameter keyed off `EcosReply::Kind::REPLY` (assuming a direct answer to our own `get()`/`set()`/`request()` is never a real echo) - live evidence showed this was unreliable (Ecos's own REPLY/EVENT framing for `get()` replies didn't match that assumption cleanly), so it was replaced with a fully deterministic check instead: `baseline_query_object_id` (which the bridge itself controls precisely) rather than guessing at Ecos's wire framing.
  - The deeper form of the same bug: live testing with both WLANmaus and MultiMaus simultaneously connected showed XpressNet's own changes never reaching Z21, and Z21's own changes never reaching XpressNet, even though each direction to/from Ecos worked individually. Root cause: `handleEcosCommand()`/`handleEcosFunctionCommand()`'s echo check was all-or-nothing - correct for not echoing (say) XpressNet's own command back to XpressNet, but with a *third* protocol now in play, dropping the entire update also threw away the copy meant for Z21 (or vice versa). Fixed properly: removed the early-return echo gate from both handlers entirely, and instead added `CommandRouter::wasRecentSource(address, source)`, called individually per destination inside `broadcastCommand()`'s `ECOS` branch - skip forwarding only to whichever protocol was the actual recent originator, always forward to the other one. This is a real architectural fix, not a patch: it generalizes correctly to however many throttle-facing protocols this bridge ever ends up supporting (LocoNet included, when that's built). The now-fully-redundant, separately-buggy `EcosInterface`-level echo mechanism (`echo_queue`/`addToEchoQueue()`/`isEchoCommand()`, `ECHO_TYPE_SPEED`) was removed as dead code in the same pass, since `wasRecentSource()` supersedes what it was trying to do.
  - **Confirmed live** (after the architectural fix): WLANmaus drive/direction/function reaching Ecos; Ecos-direct changes reaching both WLANmaus and MultiMaus (including MultiMaus's "stolen" icon refresh); MultiMaus's own changes reaching Ecos. **Not yet confirmed**: XpressNet's changes reaching Z21 through Ecos, and Z21's changes reaching XpressNet through Ecos - the last live attempt showed Ecos itself reporting `control[none]` for the test loco object right after a bridge-issued command, which reads as leftover per-object control/view registration state on the Ecos side from this session's unusually heavy repeated subscribe/query/reconnect cycling, not a code issue. Session paused here at the user's request rather than continuing to debug what looks like accumulated Ecos-side state; next session should restart Ecos (or at least deselect/reselect the test loco there) for a clean baseline before re-verifying.

4 new native tests (`ecosBuildGetPropertyCmd`/`ecosBuildGetFunctionCmd`); native suite 254/254 passing throughout every fix. No version bump or git tag this session, per the standing convention - cross-protocol propagation is still unconfirmed, and a tag should represent proven-working state, not just "extensively improved."
